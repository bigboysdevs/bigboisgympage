...
type HeroFigureVariant = 'classic' | 'video' | 'nav';
...
const NAV_ROOT = 'navbar-brand-figure-root relative h-full w-full overflow-visible pointer-events-none';
const NAV_SLOT = 'navbar-brand-figure-canvas relative h-full w-full overflow-visible';
const NAV_VIEWPORT =
  'navbar-brand-figure-viewport relative h-full w-full overflow-visible flex items-end justify-center';
...
function TinyLoader({ compact = false }: { compact?: boolean }) {
  return (
    <div
    ...
    >
      <div
        className={[
          'animate-spin rounded-full border-2 border-white/15 border-t-red-600/80 motion-reduce:animate-none',
...
export default function HeroFigureOnly({
  ...
  if (isNav) {
    if (mode === 'deciding') {
      return <div className={`${NAV_ROOT} ${NAV_SLOT}`} aria-hidden />;
    }

    if (mode === 'static') {
      return (
        <div className={NAV_ROOT}>
          <div className={NAV_SLOT}>
            <div className={NAV_VIEWPORT}>
              <img
                src={HERO_FIGURE_FALLBACK_PNG}
                alt=""
                className="navbar-brand-figure-fallback h-full w-full object-contain object-bottom"
                draggable={false}
              />
            </div>
          </div>
    ...
    return (
      <div className={NAV_ROOT}>
        <div className={NAV_SLOT}>
          <Suspense fallback={<TinyLoader compact />}>
            <div className={NAV_VIEWPORT}>
              <HeroLogo3D
                modelUrl={modelUrl}
                performanceMode
                figureScreenOffset={0}
                enableRotate={false}
                variant="nav"
              />
            </div>
